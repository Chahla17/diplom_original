...
from .models import Item, OrderItem, Order, Category, Item2
...
class HomeView3(ListView):
    model = Item
    paginate_by = 9
    template_name = "item_list_2.html"
    context_object_name = 'items'
    
class HomeView4(ListView):
    model = Item
    paginate_by = 9
    template_name = "item_list_3.html"
    context_object_name = 'items'
...
class CheckoutView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        order = Order.objects.get(user=self.request.user, ordered=False)
        order.ordered = True
        order.first_name = args[0].POST["first_name"]
        order.dosatvka = args[0].POST["payment"]
        order.save()
